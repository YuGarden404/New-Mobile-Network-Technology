...
class TrafficSimulatorWiFi8:
    ...
    def generate_dynamic_slices(self, n_slices):
        ...
        interferences = np.zeros(n_slices) # 0 表示无干扰
        ...
        # 随机让 10% ~ 20% 的切片遭遇强干扰 (模拟它们被分配的频率突然被雷达等干扰)
        num_interfered = max(1, int(n_slices * np.random.uniform(0.1,0.2)))
        interfered_indices = np.random.choice(n_slices, num_interfered, replace=False)

        for idx in interfered_indices:
            # 干扰强度设定在 0.5 到 0.95 之间（代表物理信道容量瞬间丢失 50% ~ 95%）
            interferences[idx] = np.random.uniform(0.5, 0.95)
        ...
        return {
            ...
            "W_qos": w_qos,                 # 延迟阈值
            "B_max": b_max,                 # 缓冲区上限
            "interferences": interferences  # 干扰数组
        }
...
if __name__ == "__main__":
    ...
    try:
        simulator = TrafficSimulatorWiFi8(test_config_path)
