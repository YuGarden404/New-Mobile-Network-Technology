...
def evaluate_and_plot():
    ...
    # 准备 CSV 记录表 (模仿 Pedro 的 ExperimentHandler)
    csv_filename = os.path.join(results_dir, "evaluation_results.csv")
    results_data = []

    # 测试的切片规模拓展性 (Scalability)
    test_slice_counts = [4, 8, 16, 32, 64]
    num_experiments_per_scale = 10  # 每个规模测试 10 次取平均

    print("\n================ 2. 开始评测 (Scalability Test) ================")
    ...
    print(f"论文图表绘制完成，保存至 {results_dir} 文件夹中 scalability_test.png 和 .pdf")
